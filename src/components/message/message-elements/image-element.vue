...
export default {
  ...
  computed: {
    ...
    imageUrl() {
      const url = this.payload.imageInfoArray[0].url || this.payload.imageInfoArray[0].URL
  ...
  methods: {
    ...
    handlePreview() {
      this.$bus.$emit('image-preview', {
        url: this.payload.imageInfoArray[0].url || this.payload.imageInfoArray[0].URL,
        urlList: this.imgUrlList
      })
    }
  }
}
